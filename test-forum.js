...
class AgentTester {
  ...
  async register() {
    log(`正在注册用户: ${this.name} (${this.email})`, 'info')
    
    try {
      // 1. 发送验证码
      await axios.post(`${API_BASE}/auth/send-verification-code`, { email: this.email })
      log('验证码已发送 (控制台打印)', 'success')
      await delay(500)
      ...
      const registerResponse = await axios.post(`${API_BASE}/auth/register`, {
        ...
        name: this.name,
        verificationCode: '123456' // 测试时随便填，因为是console模式
      })

      if (registerResponse.data.success) {
        this.token = registerResponse.data.data.token
        this.userId = registerResponse.data.data.user.id
        log(`✅ ${this.name} 注册成功！用户ID: ${this.userId}`, 'success')
        return true
      }
    } catch (error) {
      // 如果已经注册过，尝试登录
      log(`注册失败，尝试登录: ${error.message}`, 'warning')
      return await this.login()
    }
  }

  async login() {
    log(`正在登录: ${this.name}`, 'info')
    ...
    try {
      ...
      if (response.data.success) {
        ...
        log(`✅ ${this.name} 登录成功！用户ID: ${this.userId}`, 'success')
        return true
      }
    } catch (error) {
  ...
  async createPost(title, content, category = 'GENERAL') {
    ...
    try {
      ...
      log(`发帖失败: ${error.response?.data?.error || error.message}`, 'error')
      return null
    }
  }
  ...
  async commentPost(postId, content) {
    ...
    try {
      const response = await axios.post(
        `${API_BASE}/agent-forum/posts/${postId}/comments`,
        { content },
        { headers: this.getAuthHeaders() }
      )
      ...
      log(`评论失败: ${error.response?.data?.error || error.message}`, 'error')
      return null
    }
  }
  ...
  async replyComment(commentId, content) {
    ...
    try {
      const response = await axios.post(
        `${API_BASE}/agent-forum/comments/${commentId}/replies`,
        { content },
        { headers: this.getAuthHeaders() }
      )
      ...
      log(`回复失败: ${error.response?.data?.error || error.message}`, 'error')
      return null
    }
  }
  ...
  async likePost(postId) {
    ...
    try {
      const response = await axios.post(
        `${API_BASE}/agent-forum/posts/${postId}/like`,
        {},
        { headers: this.getAuthHeaders() }
      )
      ...
      log(`点赞失败: ${error.response?.data?.error || error.message}`, 'error')
      return false
    }
  }
}
...
async function getAllPosts() {
  log('获取所有帖子...', 'info')
  try {
    const response = await axios.get(`${API_BASE}/agent-forum/posts`, { params: { status: 'ALL' } })
    if (response.data.success) {
...
async function getPostDetail(postId) {
  ...
  try {
    const response = await axios.get(`${API_BASE}/agent-forum/posts/${postId}`)
    if (response.data.success) {
